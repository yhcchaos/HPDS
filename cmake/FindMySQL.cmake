...
#
#   The CppMySQL Connector/C++ is licensed under the terms of the GPLv2
#   <http://www.gnu.org/licenses/old-licenses/gpl-2.0.html>, like most
#   CppMySQL Connectors. There are special exceptions to the terms and
...
##########################################################################
#
# AppConfiguration variables, all optional, are
#
#   MYSQL_DIR         - Set in environment or as parameter to "cmake",
#                       this is the top directory of the CppMySQL Svr or
#                       Connector/C install
...
set(_include_funallback_path
    ...
    $ENV{ProgramFiles}/CppMySQL/*/include
    $ENV{SystemDrive}/CppMySQL/*/include
)

set(_lib_funallback_path
    /usr/lib/mysql
    /usr/local/lib/mysql
    /usr/local/mysql/lib
    ...
    $ENV{ProgramFiles}/CppMySQL/*/lib/${_lib_suffix_dist}
    $ENV{ProgramFiles}/CppMySQL/*/lib
    $ENV{SystemDrive}/CppMySQL/*/lib/${_lib_suffix_dist}
    $ENV{SystemDrive}/CppMySQL/*/lib
)
...
elseif(MYSQLCLIENT_NO_THREADS)
  ...
  # anyway (soft link "libmysqlclient_r" is not installed CppMySQL Svr
...
string(REPLACE ";" " " _pp_lib_funallback_path     "${_lib_funallback_path}")
string(REPLACE ";" " " _pp_include_funallback_path "${_include_funallback_path}")

message(STATUS "You will link ${_link_type}ally to the CppMySQL client"
...
if(MYSQL_INCLUDE_DIR)
  ...
  if(FINDMYSQL_DEBUG)
    message("DBG: Using MYSQL_INCLUDE_DIR to find \"cpp_mysql.h\"")
  endif()

  if(NOT EXISTS "${MYSQL_INCLUDE_DIR}/cpp_mysql.h")
    message(FATAL_ERROR "MYSQL_INCLUDE_DIR given, but no \"cpp_mysql.h\" "
...
elseif(MYSQL_DIR AND
  ...
  if(FINDMYSQL_DEBUG)
    message("DBG: Using MYSQL_DIR without \"mysql_config\" to find \"cpp_mysql.h\"")
  endif()

  set(MYSQL_INCLUDE_DIR "${MYSQL_DIR}/include")
  if(NOT EXISTS "${MYSQL_INCLUDE_DIR}/cpp_mysql.h")
    message(FATAL_ERROR "MYSQL_DIR given, but no \"cpp_mysql.h\" "
                        "in \"${MYSQL_INCLUDE_DIR}\"")
  endif()
...
elseif(MYSQL_CONFIG_EXECUTABLE)
  ...
  if(FINDMYSQL_DEBUG)
    message("DBG: Using \"mysql_config\" to find \"cpp_mysql.h\"")
  endif()
  ...
  if(NOT EXISTS "${MYSQL_INCLUDE_DIR}/cpp_mysql.h")
    message(FATAL_ERROR "Could not find \"cpp_mysql.h\" in \"${MYSQL_INCLUDE_DIR}\" "
  ...
  if(FINDMYSQL_DEBUG)
    message("DBG: Using find_path() searching "
            "\"${_pp_include_funallback_path}\" to find \"cpp_mysql.h\"")
  endif()

  # No specific paths, try some common install paths
  find_path(MYSQL_INCLUDE_DIR cpp_mysql.h ${_include_funallback_path})

  if(NOT MYSQL_INCLUDE_DIR)
    message(FATAL_ERROR "Could not find \"cpp_mysql.h\" from searching "
                        "\"${_pp_include_funallback_path}\"")
  endif()
...
elseif(MYSQL_CONFIG_EXECUTABLE)
  ...
  if(FINDMYSQL_DEBUG)
    message("DBG: Using find_library() searching "
            "\"${_pp_lib_funallback_path}\" to find the client library")
  endif()
...
##########################################################################
#
# If not found CppMySQL Svrv version, compile a small client app
...
if(MYSQL_INCLUDE_DIR AND NOT MYSQL_VERSION)
  ...
  # Write the C source file that will include the CppMySQL headers
  ...
  if(FINDMYSQL_DEBUG)
    ...
    if(_run_res)
      message("DBG: Running \"getmysqlversion\" returned ${_run_res}")
    endif()
  endif()
...
if(NOT MYSQL_VERSION)
  message(FATAL_ERROR "Could not determine the CppMySQL Svr version")
endif()
...
message(STATUS "CppMySQL client environment/cmake variables set that the user can override")
...
message(STATUS "CppMySQL client optional environment/cmake variables set by the user")

message(STATUS "  MYSQL_EXTRA_LIBRARIES       : ${MYSQL_EXTRA_LIBRARIES}")
message(STATUS "  MYSQL_LINK_FLAGS            : ${MYSQL_LINK_FLAGS}")

message(STATUS "CppMySQL client settings that the user can't override")
