...
void test_aes256ecb() {
    for(int i = 0; i < 4097; ++i) {
        auto str = yhchaos::random_string(i);
        std::string encode;
        encode.resize(i+ 30);
        std::string decode;
        decode.resize(i+ 30);

        auto key = yhchaos::random_string(32);

        int32_t len = yhchaos::CryptoUtil::AES256Ecb(key.c_str(), str.c_str(), str.size(), &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if(len > 0) {
            encode.resize(len);
        }
        len = yhchaos::CryptoUtil::AES256Ecb(key.c_str(), encode.c_str(), encode.size(), &decode[0], false);
...
void test_aes256cbc() {
    for(int i = 0; i < 4097; ++i) {
        ...
        auto key = yhchaos::random_string(32);
        auto iv = yhchaos::random_string(16);

        int32_t len = yhchaos::CryptoUtil::AES256Cbc(key.c_str(), iv.c_str(), str.c_str(), str.size(), &encode[0], true);
        std::cout << "encode_len: " << len << std::endl;
        if(len > 0) {
            encode.resize(len);
        }
        len = yhchaos::CryptoUtil::AES256Cbc(key.c_str(), iv.c_str(), encode.c_str(), encode.size(), &decode[0], false);
        std::cout << "decode_len: " << len << std::endl;
        if(len > 0) {
            decode.resize(len);
        }
...
void test_one() {
    //std::string key = yhchaos::random_string(32);
    //std::string iv = yhchaos::random_string(16);
    //std::string str = yhchaos::random_string(125);
    ...
    int32_t len = yhchaos::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), str.c_str(), str.size(), &encode[0], true);
    if(len >= 0) {
        encode.resize(len);
    }

    int32_t len2 = yhchaos::CryptoUtil::AES128Cbc(key.c_str(), iv.c_str(), encode.c_str(), encode.size(), &decode[0], false);
    if(len2 >= 0) {
        decode.resize(len2);
    }
    ...
    std::cout << "encode: " << yhchaos::base64encode(encode) << " - len=" << encode.size() << std::endl;
...
void test_rsa() {
    ...
    //std::cout << "generate: " << yhchaos::RSACipher::GenerateKey(pubkey_path, prikey_path) << std::endl;
    yhchaos::RSACipher::ptr rsa = yhchaos::RSACipher::Create(pubkey_path, prikey_path);
    if(!rsa) {
...
int main(int argc, char** argv) {
    ...
    //    auto v = yhchaos::random_string(128, "0123456789");
    //    std::cout << "[" << v << "]" << std::endl;
    //    for(auto& n : v) {
    //        YHCHAOS_ASSERT(n);
    //    }
