#ifndef __YHCHAOS_ZK_CLI_H__
#define __YHCHAOS_ZK_CLI_H__
...
class ZKCli : public std::enable_shared_from_this<ZKCli> {
public:
    class FdEventType {
    public:
    ...
    class FlagsType {
    public:
        static const int EPHEMERAL; // = ZOO_EPHEMERAL;短暂的，瞬时的
    ...
    typedef std::shared_ptr<ZKCli> ptr;
    //watcher
    typedef std::function<void(int type, int stat, const std::string& path, ZKCli::ptr)> watcher_callback;
    ...
    int32_t setSvrs(const std::string& hosts);
    ...
    int32_t getAppConfig(std::string& val, bool watch, Stat* stat = nullptr);
