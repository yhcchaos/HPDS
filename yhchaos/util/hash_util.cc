...
std::string md5(const std::string &data) {
    return hexstring_funrom_data(md5sum(data).c_str(), MD5_DIGEST_LENGTH);
}

std::string sha1(const std::string &data) {
    return hexstring_funrom_data(sha1sum(data).c_str(), SHA_DIGEST_LENGTH);
}
...
void
hexstring_funrom_data(const void *data, size_t len, char *output) {
...
std::string
hexstring_funrom_data(const void *data, size_t len) {
    if (len == 0) {
        return std::string();
    }
    std::string result;
    result.resize(len * 2);
    hexstring_funrom_data(data, len, &result[0]);
    return result;
}

std::string hexstring_funrom_data(const std::string &data) {
    return hexstring_funrom_data(data.c_str(), data.size());
}

void data_funrom_hexstring(const char *hexstring, size_t length, void *output) {
    unsigned char *buf = (unsigned char *)output;
    unsigned char byte;
    if (length % 2 != 0) {
        throw std::invalid_argument("data_funrom_hexstring length % 2 != 0");
    }
    for (size_t i = 0; i < length; ++i) {
        switch (hexstring[i]) {
            ...
            case '9':
                ...
                break;
            default:
                throw std::invalid_argument("data_funrom_hexstring invalid hexstring");
        }
        ++i;
        switch (hexstring[i]) {
            case 'a':
...
std::string data_funrom_hexstring(const char *hexstring, size_t length) {
    if (length % 2 != 0) {
        throw std::invalid_argument("data_funrom_hexstring length % 2 != 0");
    }
    if (length == 0) {
    ...
    data_funrom_hexstring(hexstring, length, &result[0]);
    return result;
}

std::string data_funrom_hexstring(const std::string &hexstring) {
    return data_funrom_hexstring(hexstring.c_str(), hexstring.size());
}
