#ifndef __YHCHAOS_DB_DB_H__
#define __YHCHAOS_DB_DB_H__
...
class IMySQLData {
public:
    typedef std::shared_ptr<IMySQLData> ptr;
    virtual ~IMySQLData() {}
...
public:
    virtual ~IMySQLQuery() {}
    virtual IMySQLData::ptr query(const char* format, ...) = 0;
    virtual IMySQLData::ptr query(const std::string& sql) = 0;
};

class IMySQLStmt {
public:
    typedef std::shared_ptr<IMySQLStmt> ptr;
    virtual ~IMySQLStmt(){}
    ...
    virtual int64_t getLastInsertId() = 0;
    virtual IMySQLData::ptr query() = 0;

    virtual int getErrno() = 0;
    virtual std::string getErrStr() = 0;
};

class IMySqlTrans : public IMySQLUpdate {
public:
    typedef std::shared_ptr<IMySqlTrans> ptr;
    virtual ~IMySqlTrans() {};
...
class IMySQLDB : public IMySQLUpdate
            ,public IMySQLQuery {
public:
    typedef std::shared_ptr<IMySQLDB> ptr;
    virtual ~IMySQLDB() {}

    virtual IMySQLStmt::ptr prepare(const std::string& stmt) = 0;
    virtual int getErrno() = 0;
    virtual std::string getErrStr() = 0;
    virtual IMySqlTrans::ptr openTransaction(bool auto_commit = false) = 0;
};
