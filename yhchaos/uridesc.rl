...
%%{
    ...
    action save_scheme
    {
        //fpc是现在解析到的末尾的位置,把两个长度一减就是scheme
...
//用有限状态机去解析uristr
UriDesc::ptr UriDesc::Create(const std::string& uristr) {
    UriDesc::ptr uri(new UriDesc);
    //给有限状态机用的变量
    int cs = 0;
    const char* mark = 0;
    %% write init;
...
bool UriDesc::isDefaultPort() const {
    if(m_port == 0) {
        return true;
    }
    if(m_scheme == "http"
...
const std::string& UriDesc::getPath() const {
    static std::string s_default_path = "/";
    return m_path.empty() ? s_default_path : m_path;
}

int32_t UriDesc::getPort() const {
    if(m_port) {
        return m_port;
    }
    if(m_scheme == "http"
...
//讲解析好的转换为uri
std::ostream& UriDesc::dump(std::ostream& os) const {
    os << m_scheme << "://"
       << m_userinfo
       << (m_userinfo.empty() ? "" : "@")
       << m_host
...
std::string UriDesc::toString() const {
    std::stringstream ss;
    dump(ss);
    return ss.str();
}

NetworkAddress::ptr UriDesc::createNetworkAddress() const {
    auto addr = NetworkAddress::SearchForAnyIPNetworkAddress(m_host);
    if(addr) {
