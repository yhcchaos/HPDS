...
int ZlibStream::read(ByteBuffer::ptr ba, size_t length) {
    throw std::logic_error("ZlibStream::read is invalid");
}
...
int ZlibStream::write(ByteBuffer::ptr ba, size_t length) {
...
int ZlibStream::init(Type type, int level, int window_bits
    ...
    YHCHAOS_ASSERT((level >= 0 && level <= 9) || level == DEFAULT_COMPRESSION);
    YHCHAOS_ASSERT((window_bits >= 8 && window_bits <= 15));
    YHCHAOS_ASSERT((memlevel >= 1 && memlevel <= 9));
...
std::string ZlibStream::getRes() const {
    std::string rt;
    for(auto& i : m_buffs) {
...
yhchaos::ByteBuffer::ptr ZlibStream::getByteBuffer() {
